...
local Combos = {"Neutral", "Forward", "Block", "Forward Block", "Heavy", "Slide", "Aerial", "Wall", "Finisher", "Damage", "Hits"};
...
local function buildComboIcon(Name, Combo, Num)
    ...
    icon = '<div style="text-align: center; vertical-align: top; white-space: nowrap; display: inline-block; padding: 0.15em 0.15em 0;">'..Icon._Melee(iconShape, iconType, 'x19')..iconNote..icon..'</div>';
    return icon;
end
...
local stancelegend = {
	...
	table.concat {
        ...
        Icon._Melee('DEFAULT', '360', 'x19'),'&nbsp;360°&nbsp;Attack&nbsp;&nbsp;&nbsp;',
        Icon._Melee('DEFAULT', 'Slam', 'x19'),'&nbsp;Slam&nbsp;Attack&nbsp;&nbsp;&nbsp;',
    ...
    -- Key Inputs table
        ...
        '\n|+ Key Inputs',
        '\n| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0em;" | <small>',Icon._Melee('DEFAULT', '', 'x19'),'&nbsp;Basic Melee&nbsp; (PC default LMB or E)</small>',
        '\n|-',
        '\n| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0em;" | <small>', Icon._Melee('Heavy', '', 'x19'), '&nbsp;[[Melee#Heavy Attack|Heavy Attack]]&nbsp; (PC default MMB)</small>', 
        '\n|-', 
        '\n| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0em;" | <small>', Icon._Melee('Block', '', 'x19'), '&nbsp;[[Melee#Blocking|Block]]&nbsp; (PC default RMB)</small>', 
        '\n|-', 
        '\n| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0em;" | <small>', Icon._Melee('Up', '', 'x19'), '&nbsp;Forward Movement&nbsp; (PC default W)</small>', 
        '\n|}',
...
local function buildStanceLegend(colSpan)
    local s = {}
	for i, v in ipairs(stancelegend) do
		table.insert(s, v)
		if i ~= #stancelegend then table.insert(s, colSpan) end
	end
...
local function calcAvgDmgMulti(comboData)
    ...
    multiplierAvg = (math.floor(multiplierAvg * 100 + 0.5) / 100);
...
local function buildComboRow(Name, comboType)
    ...
    if stanceData[2][comboType] then
    ...
    tRows = tRows..
        '\n|- id="'..comboData.Name..'" style="background-color:'..color..'"'..
    ...
    for j = 1, maxCols do
        ...
        if j <= numIcons then
            tRows = tRows..'\n| rowspan = "'..rowSpan..'" style="vertical-align:top; text-align:center; border-left: none;'..bRight..'" | '..buildComboIcon(Name, comboType, j);
        else
    ...
    if comboData["Duration"] ~= nil then
        tRows = tRows..'\n| rowspan = "'..rowSpan..'" style="padding: 0.75em 0.5em; line-height: '..lineHeight..'%; border-bottom: none;" |'..comboData["Duration"]..'s';
    else
        tRows = tRows..'\n| rowspan = "'..rowSpan..'" style="padding: 0.75em 0.5em; line-height: '..lineHeight..'%; border-bottom: none;" | N/A';
    end
    
    -- display average damage multiplier per second
    tRows = tRows..'\n| rowspan = "'..rowSpan..'" style="padding: 0.75em 0.5em; line-height: '..lineHeight..'%; border-bottom: none;" |'..calcAvgDmgMulti(comboData)..'%/s';
    
    -- if combo has required preliminary key inputs, add a row below to display the inputs
    if newLine then
        tRows = tRows..'\n|- style="background-color:'..color..'"';
        if comboType == 'Heavy' or comboType == 'Slide' or comboType == 'Aerial' or comboType == 'Wall' or comboType == 'Finisher' then
            tRows = tRows..'\n| style="vertical-align: top; padding: 0.25em 0 0.75em; line-height: '..lineHeight..'%; text-align: center; border-top: none;" |';
        else
            tRows = tRows..'\n| style="vertical-align: top; padding: 0 0 0.25em; line-height: '..lineHeight..'%; text-align: center; border-top: none;" |';
        end
...
-- building PvE stance comparison table
		...
		if (stanceName ~= "Legend" and stanceData["ConclaveOnly"] == nil) then
			for comboType, comboData in pairs(stanceData) do
				if (comboType ~= "WeaponType") then
					if (displayAllCombos or Shared.contains(uniqueCombos, comboType, false)) then
						local duration = stanceData[comboType]["Duration"] or 'N/A';
						local weaponType = stanceData["WeaponType"] or stanceName;
						local totalDmgMulti = calcTotalDmgMulti(stanceData[comboType]);
						local slashDmgMulti = calcTotalSlashProcMulti(stanceData[comboType]);
						tRows = tRows..'\n| '..stanceName..
							'\n| '..weaponType..
							'\n| '..comboType..
