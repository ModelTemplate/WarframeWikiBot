...
local function buildDamageTypeComparisonString(Attack1, Attack2)
    ...
    for i, element in ipairs(Elements) do
        ...
        if (damage1 ~= nil or damage2 ~= nil) then
        	...
        	result = result..buildCompareString(damage1, damage2, Icon._Proc(element, "text").." damage", {2, 1}, nil, {"Higher", "Lower"}, "\n***")
        end
    end
    return result
end
...
function p.buildAutoboxCategories(frame)
    ...
    elseif(HasTrait(Weapon, "Kuva Lich")) then
        result = result.."[[Category:Kuva]]"
    elseif(HasTrait(Weapon, "Prisma")) then
    	result = result.."[[Category:Prisma]]"
    end
    ...
    elseif(HasTrait(Weapon, "Baro")) then
    	result = result.."[[Category:Baro Ki'Teer Offering]]"
	elseif(HasTrait(Weapon, "Syndicate")) then
		result = result.."[[Category:Syndicate Offerings]]"
    end
	
	if(HasTrait(Weapon, "Invasion Reward")) then
		result = result.."[[Category:Invasion Reward]]"
	end
...
function p.getRivenDispositionTable(frame)
	...
	for i = 1, 5 do
		...
		table.insert(result, Icon._Dis(ranges[i]/1000))
	end
    ...
    for i = 1, 5 do
		...
		k = #result
		for j = ranges[i], ranges[i+1] + 1, -1 do
			k = k + 1
			result[k] = p.getRivenDispositionList({args = {WeaponType, j/1000}})
		end
	end
	table.insert(result, "\n|}")
...
local function BuildCompRow(Head, Weapon, UseCompDisplay)
    ...
    if not UseCompDisplay or Weapon.ComparisonDisplay == nil then
        ...
        if(hasAttack(Weapon, "Charge")) then
            attName = "Charge"
    --User:Kiljaedenas 2/15/21 - Attempting correction on comp tables for Area weapons to show Area damage
        elseif(hasAttack(Weapon, "Area")) then
        	attName = "Area"
        elseif(hasAttack(Weapon, "Normal")) then
            attName = "Normal"
        else
...
function p.getCompTableGuns(frame)
	...
	table.insert(Head,{{"default", "CritMultiplier"},true,"[[Critical multiplier|Crit<br/>Dmg]]"})
	table.insert(Head,{{"default", "StatusChance"},true,"[[Status Chance|Status]]"})
	table.insert(Head,{{"default", "FireRate"},true,"[[Fire Rate|Fire<br/>Rate]]"})
	table.insert(Head,{"Disposition",true,"[[Riven Mods#Disposition|Dispo]]"})
	table.insert(Head,{"Mastery",true,"[[Mastery Rank|MR]]"})
	table.insert(Head,{"Magazine",true,"[[Ammo#Magazine Capacity|Mag<br/>Size]]"})
	table.insert(Head,{"MaxAmmo",true,"[[Ammo|Ammo<br/>Cap]]"})
	table.insert(Head,{"Reload",true,"[[Reload Speed|Reload]]"})
	table.insert(Head,{{"default","BulletType"},false,"Projectile"})
	table.insert(Head,{"PunchThrough",true,"[[Punch Through|PT]]"})
	table.insert(Head,{"Accuracy",true,"[[Accuracy]]"})
	table.insert(Head,{"Introduced",true,"Intro"})
...
function p.getCompTableConclaveGuns(frame)
	...
	table.insert(Head,{"Introduced",true,"Introduced"})
...
function p.weaponTooltip(frame)
    ...
    table.insert(result, tableEnd..conclaveNotice..tableEnd.."\n</div>")
